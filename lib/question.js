...
module.exports = {
     ...
     * @param {number} variant - The variant associated with the issues.
    ...
    _writeCourseIssues(client, courseIssues, variant, authn_user_id, studentMessage, courseData, callback) {
        async.eachSeries(courseIssues, (courseErr, callback) => {
            ...
            sqldb.query(sql.get_group_work, {groupid: variant.group_id}, (err, result) => {
                if (ERR(err, callback)) return;
                if (result.rowCount != 0){          
                        params.push(result.rows[0].user_id);
                        if (client) {
                            sqldb.callWithClient(client, 'issues_insert_for_group_variant', params, (err) => {
                                if (ERR(err, callback)) return;
    ...
    */
    ...
    _buildQuestionUrls(urlPrefix, variant, question, instance_question, assessment) {
        ...
        if (variant.workspace_id) {
            urls.workspaceUrl = `/workspace/${variant.workspace_id}`;
        }
        ...
        async.series([
            (callback) => {
                if (variant_id != null) {
                    ...
                    const assessment_group_work = locals.assessment ? locals.assessment.group_work : false;
                    this.ensureVariant(locals.question.id, instance_question_id, locals.user.user_id, locals.authn_user.user_id, assessment_group_work, course_instance_id, locals.course, options, require_open, (err, variant) => {
                        if (ERR(err, callback)) return;
